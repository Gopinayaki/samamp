...
import { UntypedFormBuilder, UntypedFormGroup, Validators, UntypedFormControl, AbstractControl, ValidationErrors, AsyncValidatorFn } from '@angular/forms';
...
export class PagesaveformComponent implements OnInit {
  ...
  constructor(public toastController: ToastController, public modalController: ModalController, private fb: UntypedFormBuilder, private pageService: PagesService) { }
  ...
  ngOnInit() {
    ...
    this.controlForm.get('PageAccessType').valueChanges.subscribe(value => {
      if(value == 'E') {
        ...
        this.controlForm.addControl('MapType', new UntypedFormControl(''))
        this.controlForm.addControl('EnterpriseId', new UntypedFormControl('', [Validators.required]))
        this.controlForm.addControl('SiteId', new UntypedFormControl('', [Validators.required]))
        this.controlForm.addControl('PlantId', new UntypedFormControl('', [Validators.required]))
        this.controlForm.addControl('AreaId', new UntypedFormControl('', [Validators.required]))
        this.controlForm.addControl('UnitId', new UntypedFormControl('', [Validators.required]))
        this.valueChange()
      }
    })
    if(this.data) {
      ...
      this.pageService.getByIdPageDetails(this.data.PageId).subscribe((pagData: any) => {
        ...
        console.log("PageProperties", this.PageProperties)
