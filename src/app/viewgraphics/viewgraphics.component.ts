...
export class ViewgraphicsComponent implements OnInit {
  ...
  constructor(public toastController: ToastController, private iab: InAppBrowser,private platform: Platform, public modalController: ModalController, private router: Router, private aroute: ActivatedRoute, private pageService: PagesService) { }
  ...
  selectPageItem(event) {
    if (event.itemData.level == 6 && this.graphicsUrl != '') {
      ...
      const browser = this.iab.create(`${this.graphicsUrl}/Hist_GraphicsDisplay/GraphicsDisplay_external?username=admin&password=rq38ugbyyiQqfODPGDolMA==&pageid=${event.itemData.id}&pagename=${event.itemData.text}`, target, {
        location: 'yes',
        hidden: 'no',
